...
import { FaPlus, FaEdit, FaTrash, FaFilePdf, FaBox } from "react-icons/fa";

const emptyForm = { nama_bahan_baku: "", stok: "", biaya: "", tanggal: "" };
...
const InventoriBaku = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get("/api/bahan-baku");
      setData(res.data);
    } catch (err) {
      if (err.response?.status !== 404) console.error("Gagal memuat bahan baku:", err);
    }
  };
  ...
  const handleTambah = async () => {
    ...
    if (!form.nama_bahan_baku || !stok || !biaya || !form.tanggal) return;
    try {
      await axios.post("/api/bahan-baku", { nama_bahan_baku: form.nama_bahan_baku, stok, biaya, tanggal: form.tanggal, total: stok * biaya });
      setForm(emptyForm);
      fetchData();
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await axios.put(`/api/bahan-baku/${editForm.id}`, { nama_bahan_baku: editForm.nama_bahan_baku, stok, biaya, tanggal: editForm.tanggal, total: stok * biaya });
      setEditForm(null);
      fetchData();
  ...
  const handleHapus = async (id) => {
    ...
    try { await axios.delete(`/api/bahan-baku/${id}`); fetchData(); }
  ...
  const handlePrintPDF = () => {
    ...
    doc.autoTable({ head: [["Nama", "Stok", "Biaya", "Tanggal", "Total"]], body: data.map((i) => [i.nama_bahan_baku, i.stok, i.biaya, i.tanggal, i.total]), startY: 20 });
    doc.save("Inventori_Bahan_Baku.pdf");
  };

  const filteredData = data.filter((item) => item.nama_bahan_baku?.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-green-100 dark:border-gray-700 overflow-hidden flex flex-col">
      ...
      <div className="flex items-center gap-2.5 px-5 py-4 border-b border-green-50 dark:border-gray-700">
        ...
        <div>
          <h3 className="font-bold text-gray-800 dark:text-gray-100 text-base">Bahan Baku</h3>
      ...
      <div className="px-5 py-4 border-b border-green-50 dark:border-gray-700">
        <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Tambah Bahan Baku</h4>
        <div className="space-y-2.5">
          <InputField label="Nama Bahan Baku" name="nama_bahan_baku" placeholder="Contoh: Pupuk Urea" value={form.nama_bahan_baku}
            onChange={(e) => setForm({ ...form, nama_bahan_baku: e.target.value })} />
      ...
      <div className="flex flex-col flex-1 px-5 py-4">
        <div className="flex items-center justify-between gap-2 mb-3">
          <input type="text" placeholder="Cari nama bahan baku..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-green-50 dark:bg-gray-700 text-gray-500 dark:text-gray-300 text-xs uppercase tracking-wider">
                ...
                <th className="text-center px-3 py-2.5 font-semibold">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-green-50 dark:divide-gray-700">
              {paginatedData.length === 0 ? (
                <tr><td colSpan="6" className="text-center py-8 text-gray-400 dark:text-gray-500 text-sm">Belum ada data bahan baku</td></tr>
              ) : paginatedData.map((item) => (
                <tr key={item.id} className="hover:bg-green-50/50 dark:hover:bg-gray-700/50 transition-colors">
                  <td className="px-3 py-3 font-medium text-gray-800 dark:text-gray-200">{item.nama_bahan_baku}</td>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50 px-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <h4 className="font-bold text-gray-800 dark:text-gray-100">Edit Bahan Baku</h4>
            ...
            <div className="space-y-3">
              <InputField label="Nama Bahan Baku" value={editForm.nama_bahan_baku}
                onChange={(e) => setEditForm({ ...editForm, nama_bahan_baku: e.target.value })} />
              <div className="grid grid-cols-2 gap-2.5">
